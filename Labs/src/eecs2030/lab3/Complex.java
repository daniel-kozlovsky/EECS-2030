 ...
 * 
 * @author EECS2030 Fall 2017-18 Implementation by Daniel Kozlovsky - 214874879
 */
public final class Complex {
	...
	public int hashCode() {
		...
		return (int) (this.re() * 9999 + this.im() * 99);
	}
	...
	public static Complex valueOf(String s) {
		...
		if (parts.size() != 3 || (!parts.get(1).equals("+") && !parts.get(1).equals("-"))
			...
			throw new IllegalArgumentException("Cannot be interpreted as a complex number!");
		}

		int sign = (parts.get(1).equals("-") ? -1 : 1);

		result = new Complex(Double.valueOf(parts.get(0)), sign * Double.valueOf(parts.get(2).replace("i", "")));
