...
public class Die {
	 ...
	 * 
	 */
	public Die(String[] faces) throws IllegalArgumentException {

		if (faces.length == 0) {
			throw new IllegalArgumentException("There is at least one face per die!");
		}
	...
	public String roll() {
		...
		currentFace = rand.nextInt(NUM_FACES) + 1;
		return getValue();
	...
	public String getValue() {
		return valueMap.get(currentFace);
	...
	public SortedMap<Integer, String> getValueMap() {
		...
		return new TreeMap<Integer, String>(valueMap);
	}
	...
	public boolean equals(Object obj) {
		...
		// equal objects have the same class
		if (this.getClass() != obj.getClass()) {
			return false;
		}

		Die dieObj = (Die) obj;
		// objects have same field values
		if (NUM_FACES != dieObj.NUM_FACES) {
			return false;
		}
		Collection<String> tempCollection = dieObj.getValueMap().values();
		for (String s : this.valueMap.values()) {
			if (!tempCollection.contains(s)) {

				return false;
			}
			tempCollection.remove(s);

		}
		// same face value
		if (!getValue().equals(dieObj.getValue())) {
			return false;
		} else {
			return true;
		}
	...
	public String toString() {
		...
		for (int i = 1; i <= valueMap.size(); i++) {
			if (i != valueMap.size()) {
				dieString.append(valueMap.get(i) + ", ");
			} else {
				dieString.append(valueMap.get(i));
			}
		}
