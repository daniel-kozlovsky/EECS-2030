...
public class Lab1 {
	...
	public static int yourAge(int address, int birthYear, int birthdays) {
		...
		return lastTwoDigits((address * 2 + 42) * 50 - birthYear - 50 + birthdays - 34);
	}
	...
	public static int yourAgeChecked(int address, int birthYear, int birthdays) {
		...
		{
			throw new IllegalAddressException("Address is outside the range");
		}
		else if (birthYear < MIN_BIRTH_YEAR || birthYear > MAX_BIRTH_YEAR)
		{
			throw new IllegalBirthYearException("Birth year is outside the range");
		}
		else if (birthdays != 0 && birthdays != 1)
		{
			throw new IllegalBirthdaysException("Birthdays must be 0 or 1");
		}
		else
		{
	...
	public static void sort2(List<Integer> t) {
		...
		else if (t.get(0) < t.get(1))
		{
			int tempLarge = t.get(1);
			int tempSmall = t.get(0);
